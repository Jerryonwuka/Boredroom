...
export default function DevMailPage() {
  ...
  return (
    <main id="main" className="mx-auto max-w-3xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-fg-muted">Nothing here was sent to a real mailbox. Links point at this local server. Every template is on <a className="underline" href="/dev/emails">/dev/emails</a>.</p>
      <div className="mt-6 space-y-3">
        {messages.length === 0 ? <p className="tile p-6 text-fg-muted">No messages yet.</p> : messages.map((m) => (
          <details key={m.id} className="tile p-4">
            ...
            {m.html ? <iframe title={m.subject} srcDoc={m.html} sandbox="" className="mt-3 h-[640px] w-full rounded-[var(--radius-sm)] border border-border bg-black" /> : null}
