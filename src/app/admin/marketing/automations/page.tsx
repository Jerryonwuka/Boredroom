  ...
  return (
    <>
      ...
      <DataTable caption="Automations" className="mb-6"><thead><tr><th>Automation</th><th>Trigger</th><th>Template</th><th>State</th><th>Runs</th><th>Last run</th>{editable ? <th></th> : null}</tr></thead>
        <tbody>{autos.map((a) => <tr key={a.id}><td className="font-semibold">{a.name}</td><td className="text-sm">{LABEL[a.trigger] ?? a.trigger}{a.trigger_value != null ? ` ${a.trigger_value} day${a.trigger_value === 1 ? "" : "s"}` : ""}</td><td className="text-sm">{a.template_name}</td><td>{a.enabled ? <Badge tone="success">on</Badge> : <Badge tone="neutral">off</Badge>}</td><td className="tabular-nums">{num(a.runs)}</td><td className="text-sm text-fg-muted">{a.last_run ? relativeTime(a.last_run) : "never"}</td>{editable ? <td><details><summary className="cursor-pointer text-sm text-fg-muted">Edit</summary><div className="mt-2 w-[30rem] max-w-[70vw]"><AutomationForm automation={a} templates={templates} /></div></details></td> : null}</tr>)}</tbody>
      </DataTable>
