...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <>
      <PageHeader icon="chart-ring" title="Payments" description="Every Paystack transaction, whichever way it ended. Revenue below counts successful payments only." meta={<>Currency {m.currency} · Paystack {(await paystackConfigured()) ? "connected" : "not configured"}</>} actions={<CsvLink href="/api/admin/export?kind=payments" />} />
      {!(await paystackConfigured()) ? <Alert tone="warning" className="mb-6">Paystack is not configured. Set PAYSTACK_SECRET_KEY and add the webhook URL under Settings, Paystack; until then no payment can be taken.</Alert> : null}
