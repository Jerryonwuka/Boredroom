  ...
  return (
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="lg:col-span-2"><CardHeader title="Administrative actions" />{d.audit.length === 0 ? <p className="text-sm text-fg-subtle">None.</p> : <ul className="space-y-1 text-sm">{d.audit.map((a, i) => <li key={i}><span className="font-mono text-xs">{a.action}</span> · {a.admin_email} · {formatDateTime(a.occurred_at)}{a.reason ? <span className="text-fg-subtle"> · {a.reason}</span> : null}</li>)}</ul>}</Card>
