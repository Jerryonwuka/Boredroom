      ...
      <div className="grid gap-4 md:grid-cols-3">
        {plans.map((p) => (
          <Card key={p.id} className={p.status === "archived" ? "opacity-60" : ""}>
            <CardHeader title={<span className="flex items-center gap-2">{p.name}<Badge tone={p.status === "active" ? "success" : p.status === "hidden" ? "warning" : "neutral"}>{p.status}</Badge></span>} description={p.description ?? undefined} action={editable ? <EditSheet title={`Edit the ${p.name} plan`}><PlanForm plan={p} featureKeys={FEATURE_KEYS} /></EditSheet> : undefined} />
            <p className="font-display text-3xl">{money(p.monthly_price, p.currency)}<span className="text-sm text-fg-subtle"> / month</span></p>
            <p className="eyebrow mt-1">{money(p.annual_price, p.currency)} a year · {p.max_workspaces ? `${p.max_workspaces} workspace${p.max_workspaces === 1 ? "" : "s"}` : "unlimited workspaces"} · {p.max_users ? `${p.max_users} people` : "unlimited people"} · {p.max_storage_bytes ? bytes(p.max_storage_bytes) : "unlimited storage"} · {p.trial_days ? `${p.trial_days}-day trial` : "no trial"}</p>
