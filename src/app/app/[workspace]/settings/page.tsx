...
export default async function SettingsPage({ params, searchParams }: { params: Promise<{ workspace: string }>; searchParams: Promise<{ setup?: string }> }) {
  ...
  return (
    <AppShell ctx={ctx} counts={counts} teams={navTeams}>
      <PageHeader back={{ href: `/app/${ctx.org.slug}/dashboard`, label: "Dashboard" }} title="Settings" description="Values here live in policy and schedule records, not in the interface. Changes are audited." />
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-display text-lg">Screen recording</h2><Badge tone={policy?.recording_mode === "disabled" ? "danger" : "success"} dot>{policy?.recording_mode === "disabled" ? "Off" : policy?.recording_mode === "optional" ? "On (each person's choice)" : "On, required on marked tasks"}</Badge></div>
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-display text-lg">AI assistant</h2><Badge tone={ai.source === "none" ? "warning" : "success"} dot>{ai.source === "none" ? "Not connected (built-in parser)" : ai.source === "organisation" ? `Connected · Claude (${ai.model})` : "Connected via server key"}</Badge></div>
