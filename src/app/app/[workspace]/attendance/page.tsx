...
export default async function AttendancePage({ params, searchParams }: { params: Promise<{ workspace: string }>; searchParams: Promise<{ tab?: string; date?: string; team?: string; view?: string; month?: string }> }) {
  ...
  const viewSwitch = (view: "day" | "month", active: boolean) => (
    <Link href={`${base}/attendance?view=${view}${teamId ? `&team=${teamId}` : ""}`} aria-current={active ? "page" : undefined} className={cn("whitespace-nowrap rounded-full px-3.5 py-1.5 text-sm font-medium transition-colors duration-[var(--duration-fast)]", active ? "border border-accent/50 bg-accent-soft text-accent" : "text-fg-muted hover:text-fg")}>{view === "day" ? "Day" : "Month"}</Link>
  );
