...
export function AcknowledgePolicy({ orgSlug, next }: { orgSlug: string; next?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <label className="flex items-start gap-3 text-sm"><input type="checkbox" className="mt-0.5" checked={checked} onChange={(e) => setChecked(e.target.checked)} /> <span>I have read this version of the notice. (This records what you were shown and when; it does not by itself settle every legal question about monitoring.)</span></label>
  ...
  return (
    <form className="mt-2 grid gap-2" onSubmit={async (e) => {
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="text-sm"><span className="sr-only">Decision</span><select name="decision" value={decision} onChange={(e) => setDecision(e.target.value)} className="field">{options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label>
