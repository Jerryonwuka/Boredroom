...
export function Avatar({ profileId, name, avatarKey, size = 32, className }: { profileId: string; name: string; avatarKey?: string | null; size?: number; className?: string }) {
  ...
  return <span aria-hidden style={style} className={cn("inline-flex shrink-0 items-center justify-center rounded-full border border-border bg-[linear-gradient(180deg,var(--avatar-top),var(--avatar-bottom))] font-semibold text-fg-muted", className)}>{initialsOf(name)}</span>;
}
