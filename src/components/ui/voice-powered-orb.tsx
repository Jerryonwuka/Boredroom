...
/**
 * A glowing orb (OGL shader) that turns and ripples with the person's voice. Used while dictating.
 * With voice control on it opens the microphone itself (separately from speech recognition) and reads
 * the level from an analyser; nothing is recorded or sent. Hue 0 is the original violet; the default
 * here shifts it to Boredroom's orange.
 */
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => { onVoiceRef.current = onVoiceDetected; });

  useEffect(() => {
    const container = ctnDom.current;
    if (!container) return;

    /** RMS of the frequency data, boosted by the sensitivity, clamped to 0..1. */
    const analyzeAudio = () => {
    ...
    try {
      ...
      if (enableVoiceControl) void initMicrophone().then((ok) => { if (!disposed) micReady = ok; else stopMicrophone(); });
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && micReady) {
          ...
          // A slow idle turn so the orb is never frozen.
          currentRot += dt * 0.08;
          program.uniforms.hover.value = 0;
          program.uniforms.hoverIntensity.value = 0;
        }
        if (detected !== lastDetected) { lastDetected = detected; onVoiceRef.current?.(detected); }
