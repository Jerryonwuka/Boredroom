    ...
    <div className="space-y-4">
      ...
      {canEdit ? (
        <div className="chip flex flex-wrap items-end gap-4 p-4">
          {target === "live" ? <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={waitlistOpen} onChange={(e) => setWaitlistOpen(e.target.checked)} className="size-4 accent-[var(--accent)]" />Keep the waitlist form available</label> : null}
          {target === "maintenance" ? <><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={appAccess} onChange={(e) => setAppAccess(e.target.checked)} className="size-4 accent-[var(--accent)]" />Existing users may still use the app</label><label className="grid gap-1 text-sm"><span>Notice shown to users</span><input value={message} onChange={(e) => setMessage(e.target.value)} className={`${inputCls} w-80`} placeholder="We are doing maintenance; back at 18:00 WAT." /></label></> : null}
...
export function GeneralSettingsForm({ value }: { value: GeneralSettings }) {
  return (
    <JsonForm path="/api/admin/settings/general" method="PUT" transform={(d) => ({ value: { platform_name: String(d.platform_name), support_email: String(d.support_email ?? ""), currency: String(d.currency) } })}>
      <div className="grid gap-3 sm:grid-cols-3"><label className="grid gap-1 text-sm"><span>Platform name</span><input name="platform_name" defaultValue={value.platform_name} className={inputCls} required /></label><label className="grid gap-1 text-sm"><span>Support email</span><input name="support_email" type="email" defaultValue={value.support_email} className={inputCls} /></label><label className="grid gap-1 text-sm"><span>Reporting currency</span><input name="currency" defaultValue={value.currency} maxLength={3} className={inputCls} required /></label></div>
    </JsonForm>
  );
}
...
export function LandingSettingsForm({ value }: { value: LandingSettings }) {
  return (
    <JsonForm path="/api/admin/settings/landing" method="PUT" transform={(d) => ({ value: { headline: String(d.headline ?? ""), subheadline: String(d.subheadline ?? ""), cta: String(d.cta ?? "") } })}>
      <div className="grid gap-3"><label className="grid gap-1 text-sm"><span>Waitlist headline <span className="text-fg-subtle">(blank keeps the default)</span></span><input name="headline" defaultValue={value.headline} className={inputCls} /></label><label className="grid gap-1 text-sm"><span>Subheadline</span><input name="subheadline" defaultValue={value.subheadline} className={inputCls} /></label><label className="grid gap-1 text-sm"><span>Button text</span><input name="cta" defaultValue={value.cta} className={`${inputCls} w-64`} /></label></div>
    </JsonForm>
  );
}
...
export function BillingSettingsForm({ value }: { value: BillingSettings }) {
  return (
    <JsonForm path="/api/admin/settings/billing" method="PUT" transform={(d) => ({ value: { trial_days: Number(d.trial_days), grace_days: Number(d.grace_days) } })}>
      <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-sm"><span>Default trial, days</span><input name="trial_days" type="number" min={0} max={365} defaultValue={value.trial_days} className={inputCls} /></label><label className="grid gap-1 text-sm"><span>Grace after period end, days</span><input name="grace_days" type="number" min={0} max={60} defaultValue={value.grace_days} className={inputCls} /></label></div>
    </JsonForm>
  );
}
...
export function AdminCreateForm() {
  return (
    <JsonForm path="/api/admin/admins" transform={(d) => ({ email: String(d.email), role: String(d.role) })} submitLabel="Add administrator" successMessage="Added.">
      <div className="grid gap-3 sm:grid-cols-[1fr_220px]"><label className="grid gap-1 text-sm"><span>Account email <span className="text-fg-subtle">(an existing Boredroom account)</span></span><input name="email" type="email" className={inputCls} required /></label><label className="grid gap-1 text-sm"><span>Role</span><select name="role" defaultValue="support" className={inputCls}>{ADMIN_ROLES.map((r) => <option key={r} value={r}>{ROLE_LABEL[r]}</option>)}</select></label></div>
    </JsonForm>
  );
}
