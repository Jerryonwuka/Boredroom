...
export function QuestionsMarquee() {
  const row = (hidden: boolean) => QUESTIONS.map((q) => <li key={q} aria-hidden={hidden || undefined} className="lp-glass lp-muted mr-4 shrink-0 rounded-2xl px-6 py-4 font-display text-xl line-through decoration-1 decoration-[rgba(255,255,255,0.3)] md:text-2xl">{q}</li>);
  return (
