...
## Owner feedback round 6 (18 September 2026): messaging
...
## Owner feedback round 8 (18 September 2026): dictation and handing tasks upwards
...
## Aceternity UI component pack (23 September 2026)

The owner asked for the Aceternity pack from 21st.dev (`npx @21st-dev/cli add @aceternity`). That command delegates to `shadcn add` for 268 items, 161 of which are Aceternity Pro and need a paid token (the registry answers 401), and its dependency list contains two entries the package manager cannot install (`motion/react` is an import path, `@react-three/fiber@alpha` would replace the stable version), so the CLI run fails before writing a file. Instead the 107 free items were fetched from the same registry (`https://ui.aceternity.com/registry/<name>.json`) and written to `src/components/aceternity/` (plus `src/hooks/use-outside-click.tsx` and `src/data/globe.json` from ui.aceternity.com), with cross-imports rewritten so the hand-written kit in `src/components/ui/` is untouched. `components.json` records the registry for future single-item adds. Runtime dependencies added: three, @react-three/fiber, @react-three/drei, three-globe, @tabler/icons-react, @tsparticles/* (pinned to v3, the API the components target), react-syntax-highlighter, react-dropzone, @radix-ui/react-hover-card, mini-svg-data-uri, qss, simplex-noise, dotted-map, next-themes. Fourteen files needed small React 19 typing fixes (nullable refs, `React.JSX`, `useRef(null)`, and a typed `Component` alias where a bare `React.ElementType` collapses JSX props to `never`); the folder is excluded from ESLint as vendored code. Lint, typecheck, `next build` and 22 unit tests pass. None of these components is used by a product screen yet; the design rules in `docs/design-system.md` still apply to anything built with them.

## Landing page: a replica of resend.com's structure and feel, for Boredroom (23 September 2026)

The owner asked to discard the product design system on the landing page and replicate resend.com (sections, layout, animations, feel) with Boredroom's colours, using the 3D icons in `~/Documents/icons`. The page now has its own style layer (`.lp` classes at the end of `src/app/globals.css`: true black, white and grey type, hairline borders, glass cards, orange pill buttons, a lit 64px icon tile above section titles) and lives in `src/app/page.tsx` plus `src/components/landing/`. Sections in Resend's order, each with a Boredroom equivalent: nav with a Get started pill; announcement pill; hero with a two-line 104px headline over a turning globe with orange arcs between the cities a remote team works from (`globe.tsx` from the pack, dynamically imported); the questions a lead stops sending as a strike-through marquee where Resend has its logo strip; "Get everyone in" (join code, link and invitation cycling in a tabbed card, with role chips where Resend lists SDKs); "Built for the people doing the work" (a to-do typed live and a feed of the day's events streaming in, like Resend's send demo); two panels for recording-by-consent and live events (like Test mode and Modular webhooks, with the event list highlighting one row at a time); "A day planned in one line" with a My Day mock; "Go beyond the timer" with attendance and reports cards; "Ask, with the task attached" with a tabbed messages mock; a nine-item fairness grid with the 3D icons; the monitoring notice as the one quote; "Everything in view" with three moving-light cards and a dashboard mock whose chart draws itself (the metrics screenshot the owner sent); "A week in the room" as three vertical marquee columns of recorded moments where Resend has testimonials; FAQ; a closing hero repeat with glow; a four-column footer with a status pill. Every reveal is Resend's blur-to-sharp rise (`Reveal`, `RevealGroup`). Reduced motion stops the globe, marquees, feed and highlights. The 20 icons are copied to `public/icons/` at 320px with semantic names (a zsh one-based array shifted them by one on the first copy; redone in node). Verified: lint (only `<img>` warnings), typecheck, no console errors, no broken images, no horizontal overflow at 1024px and 375px. No fabricated testimonials or logos.

Second pass, same day, on the owner's notes: the fairness section became a hairline table (`.lp-table`/`.lp-cell`, 1px gaps over the line colour) keeping the 3D icons, with a hover that tints the cell, lifts and tilts the icon and warms the title; every card is now glass (`.lp-card`: translucent wash, 18px backdrop blur, top highlight) through `GlassCard`, which adds Aceternity's `GlowingEffect` (border glow following the pointer, recoloured orange) and the `3d-card` tilt; the hero uses `background-ripple-effect` (a clickable cell grid, recoloured) behind the copy, `world-map` (dotted map with orange arcs, `next-themes` removed in favour of a `theme` prop) in place of the globe, and `flip-words` on the last word of the headline; the dashboard mock sits inside `container-scroll-animation` (frame restyled to glass) so it tilts up as you scroll; and the whole page has a fixed sky behind it (`SiteBackground`: `stars-background`, orange `shooting-stars`, two drifting aurora blobs), with `.lp` made transparent so the glass has something to blur. Vendored patches beyond types: ripple colours, world-map theme prop, container-scroll frame classes, glowing-effect gradient.

Third pass, same day: the hero copy is now the `container-scroll-animation` title and the dashboard mock its card, so the dashboard rises to the header on scroll (the world map is gone, and the dashboard no longer repeats lower down); the flipping word is only "doing" and "stuck on"; the 3D tilt is a third of its original angle everywhere and off in the "Built for the people doing the work" cards; the star and shooting-star layers are gone (the aurora stays); a `floating-dock` of section links with the 3D icons sits at the bottom (desktop) or as a toggle (phones), restyled to glass; the marquee heading is a `hover-border-gradient` badge (orange); and the buttons are Aceternity's Shimmer (primary, orange) and Border Magic (secondary, spinning conic border via `@property`), written as CSS so the markup stayed the same.

Fourth pass (24 September 2026): glass on every surface (`.lp-glass` for rows, chips, inputs, tab strips, pills, header, footer), which exposed a compiler quirk: writing `-webkit-backdrop-filter` by hand made Lightning CSS drop the standard `backdrop-filter`, so no blur had ever reached Chrome; the manual prefixes are gone and the compiler adds its own. Then on the owner's notes: the live feed keeps a fixed four-row window so the card never changes height (the page used to jump as rows arrived); the marquee heading is plain text again and each question is a glass pill; the dock circles are glass; the outlined button is a still glass pill with an orange outline (only the filled buttons animate); the "Everything in view" cards have an opaque face again; and the hero ripple grid receives the pointer (the scroll container was swallowing hover and clicks) and its lines are brighter. The badge caused a hydration error (a div inside a p) before it was removed. Later the same day the owner reversed two of these: the floating dock is removed, and every card, cell and row is an opaque dark surface again with no backdrop blur anywhere on the page (only the sticky header keeps a blur).

## Design system v2 (24 September 2026)

The owner asked to turn the landing page's look into a design system and then rebuild the platform on it. Step one is done: `docs/design-system.md` is rewritten as the v2 spec (true black canvas, hairlines at 7/12/18%, solid dark gradient cards with a top highlight, one orange, Cal Sans and Manrope, the 3D icon set for places and lucide for controls, one dark bordered surface for every button, no glass). The product tokens in `globals.css` now carry those values and `.tile` is the landing card, so every existing screen picked up the new surfaces, borders, text tiers, radii and table styling at once. New primitives: `StatCard` (verdict cards), `Tabs` (sliding pill tabs), `HairlineGrid`/`GridCell`, `Icon3D`/`IconTile` (typed 3D icon set); `PageHeader` takes a 3D icon, `EmptyState` an `icon3d`, `Button` uses the shimmer and outline styles, the sidebar's active item is a white pill with an orange icon. The skill in `.claude/skills/boredroom-ui/` now enforces v2. Lint, typecheck, unit tests and `next build` pass.

Step two, the rebuild (same day): every workspace page was rebuilt on the v2 primitives, keeping its data and logic. Every page opens with a `PageHeader` carrying its 3D icon; the Dashboard, Workroom, Attendance (day and month), Reports and the person page open with three `StatCard` verdicts in place of number tiles; Tasks, Attendance and People use the sliding pill `Tabs`; filters are chips; underlined links became outline or subtle buttons; every empty state carries a 3D icon and one action; middle dots and arrows in meta text became commas and words across pages, forms and panels; the Workroom cards lead with the clock in the display face; the task page's figures sit in panels; the messages inbox is a tile with quiet white selection; the sidebar, workspace picker and auth shell follow the same surfaces. Verified: typecheck, lint (only `<img>` warnings on the landing page), the smoke run of all fifteen page read models against Neon, 22 unit tests, and `next build`. Signed-in screens were not viewed in a browser here (no password is typed by the assistant); the owner reviews them. Playwright was not run because it needs the local test database, which is not started while the app runs on Neon.

## Slow pages on Neon (24 September 2026)

The owner reported pages hanging for seconds after sign-in. Measured, not guessed: render time per page is 3 to 90 ms; the rest is database round trips. From the owner's laptop in Lagos to Neon in US East a round trip is about 225 ms and a new connection 1.8 s. Each service call ran in its own transaction (BEGIN, set identity, query, COMMIT: four round trips, about a second), and a dashboard load chained a session lookup, a membership lookup, three shell lookups, a policy check and three page read models (one of them six statements) — 6 to 8 s, and 16 s when an idle pooled socket had been dropped by the network (the `read ETIMEDOUT` storms in the log) and had to reconnect. Changes: the pool now uses TCP keep-alive and handles idle-client errors; `withCtx` sends BEGIN and the identity settings as one statement (values validated before inlining); `orgContext` does the session and membership lookups in one transaction and is deduplicated per request with React `cache`, as is `getCurrentUser`; the app shell's counts, teams and policy check are one statement (`workspaceShell`); the dashboard read model is two statements instead of six; the attendance board is one statement instead of three. Measured on the same connection: the shell went from about 1.5 s to 0.7 s, the dashboard's data stage from 4.6 s to 3.1 s, and the two lookups before every page from about 2 s to under 1 s, so a dashboard load drops from 6 to 8 s to roughly 4 s. The floor from here is distance: eleven round trips at 225 ms is 2.5 s whatever the code does. The fix for that is to host the app in the database's region (or choose a Neon region near the users), which turns each round trip into a millisecond and the same page into well under a second. `docs/runbooks.md` has a "Slow pages" section with the measurement command.

## Owner feedback round 12 (24 September 2026): top-right icons and a profile page

The owner asked for notifications, settings and the person to sit top right as icons that open their details on click, and for a profile page with picture, name and status. Done: `TopBar` (`src/components/app/topbar.tsx`) renders three round icon buttons on every workspace page, on the desktop header row and beside the mobile menu. The bell carries the unread count and opens the six latest notifications with "Mark read" and a link to the full page; the gear (organisation accounts only) opens links to Settings, People and teams, Review queue, Policy and Audit; the avatar opens name, title, status, email, role, "Your profile", "Switch workspace" and "Sign out". One panel is open at a time; outside click and Escape close it; toggles carry `aria-expanded`/`aria-controls`. Notifications and Settings left the sidebar, and the sidebar's account block is gone. The profile page (`/app/<workspace>/profile`) has picture upload with preview (PNG, JPEG, WebP, 2 MB, content sniffed, stored through the storage adapter under `users/<id>/`, old file deleted), name, job title and a 140-character status, plus the account and workspace list. Migration `0017_profiles.sql` adds `avatar_key`, `avatar_mime`, `title`, `status_text`, `status_set_at` to `profiles`. Routes: `GET/PATCH /api/me/profile`, `POST/DELETE /api/me/avatar`, `GET /api/avatars/:profileId` (signed-in viewers only, RLS-scoped, private cache). The session lookup now returns the avatar, title and status; `workspaceShell` returns the recent notifications in the same single statement. Verified: typecheck, lint, the fifteen smoke read models on Neon, and signed-out requests to the new routes return 401. Signed-in screens await the owner's review.
